...
public class RecyclerItemTouchHelper extends ItemTouchHelper.SimpleCallback {
    ...
    //to support left and right swipe
    public RecyclerItemTouchHelper(ToDoAdapter adapter) {
        super(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT);
        this.adapter = adapter;
    }

    //when we don't need unmove
    ...
    public void onSwiped(@NonNull final RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (direction == ItemTouchHelper.LEFT) {
            ...
            builder.setPositiveButton("Confirm",
                    new DialogInterface.OnClickListener(){ // new dialog for the confirm
                        @Override
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        super.onChildDraw(c, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive); // draw icons for edit and delete
        ...
        // dx>0 means checking if swiping left
        if (dX > 0) {
